...
parser.add_argument("-k", "--numcrossfolds", help="number of crossfolds for gridsearch if requested", required=False, type=int, default=3)
parser.add_argument("-e", "--evaluation", help="scoring for gridsearch", required=False, default="f1")
...
def search(data_obj, hyperparameter_sets, mp, cv, scoring):
    ...
    clf = create_model()
    search_results = GridSearchCV(clf, hyperparameter_sets, scoring=scoring, n_jobs=mp, cv=cv, verbose=5)
...
if __name__ == "__main__":
    ...
    if args.hyperparameters != None:
        ...
        model, score = search(dataset, search_hyperparameters, args.multiprocess, args.numcrossfolds, scoring=args.evaluation)
    else:
        model = create_model()
